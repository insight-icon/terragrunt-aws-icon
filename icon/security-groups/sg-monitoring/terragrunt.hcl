...
dependencies {
  paths = compact([local.vpc, local.global_vars["bastion_enabled"] ? "../sg-bastion" : "", local.global_vars["consul_enabled"] ? "../sg-consul" : ""])
}
...
//  create = local.global_vars["monitoring_enabled"]
  ...
  ingress_with_source_security_group_id = local.global_vars["bastion_enabled"] ? [{
    rule = "ssh-tcp"
